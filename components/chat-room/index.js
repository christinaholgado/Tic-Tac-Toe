...
import { View, Text, ScrollView, FlatList, Image, TouchableOpacity, StyleSheet, TextInput, KeyboardAvoidingView, Keyboard, TouchableWithoutFeedback, Platform, DrawerLayoutAndroidBase } from 'react-native';
...
const ChatRoom = props => {
  ...
  const updateLocation = async () => {
    ...
    //insert your update query here [update the location]
    firestore()
      .collection('chats')
      .doc(props.RoomCode)
      .collection('users')
  ...
  /*     
    ...
    //Start typing
    //Stop typing
    //one function to set to true, one to set to false
    
    //part 2 is figure out where to invoke those functions

    //onChangeText onSubmitEditing

    //onChangeText = when u start typing
    //onSubmitEditing = when u finish

    //whoever does it writes to datb
    */
        ...
        <View style={styles.innerContainer}>
          <View style={styles.headerColumn}>
            ...
            {props.RoomUsers.map((user, index) => ( user.uid !== props.Auth().currentUser.uid ? (
                <View style={styles.userContainer} key={index}>
                  ...
                  {/*make the text component conditional to user.isTyping */}
                  {user.isTyping ? (
                  <Text style={styles.isTyping}>
                    is typing
                  </Text>
                  ) : null}
                </View>
          ...
          <View style={styles.newMessage}>
            <TextInput
              ...
              value={messageText}
              onChangeText={text => {setMessageText(text); updateLocation(); isTyping();}}
              multiline={true}
              onSubmitEditing={() => {submitMessage(); notTyping();}}
              returnKeyType="done"
              blurOnSubmit={true}
              />
